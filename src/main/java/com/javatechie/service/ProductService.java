...
public class ProductService {
    ...
    //2.0 Rajout de cette méthode.
    public Product updateProductByFields(int id, Map<String, Object> fields) {
        Optional<Product> existingProduct = repository.findById(id);

        if (existingProduct.isPresent()) {
            //Comme c'est un map je peux faire un forEach.
            fields.forEach((key, value) -> {
                //Attention au reflection util il doit être du bon package.
                Field field = ReflectionUtils.findField(Product.class, key);
                field.setAccessible(true);
                //La je mets à jour le champ
